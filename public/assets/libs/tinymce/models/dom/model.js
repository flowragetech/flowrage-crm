...
(function () {
  ...
  const caretPositionFromPoint = (doc, x, y) =>
    Optional.from(doc.caretPositionFromPoint?.(x, y)).bind((pos) => {
  ...
  const run = (
    ...
    const warehouse = Warehouse.fromTable(table);
    const tableSection = Optional.from(behaviours?.section).getOrThunk(
    ...
    return output.bind((out) => {
      ...
      const tableSizing = Optional.from(behaviours?.sizing).getOrThunk(() =>
        TableSize.getTableSize(table)
      );
      const resizing = Optional.from(behaviours?.resize).getOrThunk(
        preserveTable
      );
  ...
  const elementFromGrid = (grid, row, column) => {
    const rows = extractGridDetails(grid).rows;
    return (
      Optional.from(rows[row]?.cells[column]?.element)
        .filter(isEditable)
  ...
  const getPixelForcedWidth = (editor) => {
    ...
    const parentBlock =
      dom.getParent(editor.selection.getStart(), dom.isBlock) ??
      editor.getBody();
