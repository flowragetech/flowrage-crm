  ...
  const toggleDetailsElement = (isReadonly, details, state) => {
    ...
    if (!isReadonly) {
      details.setAttribute(
        accordionReadonlyCompensationAttribute,
  ...
  const toggleAccordion = (editor, state) => {
    ...
    getSelectedDetails(editor).each((details) => {
      fireToggleAccordionEvent(
        editor,
        details,
        toggleDetailsElement(editor.readonly, details, state)
      );
    });
  };
      ...
      parser.addNodeFilter(accordionTag, (nodes) => {
        ...
        for (let i = 0; i < nodes.length; i++) {
          ...
          if (isAccordionDetailsNode(node)) {
            ...
            accordionNode.attr(
              accordionReadonlyCompensationAttribute,
              accordionNode.attr('open') === 'open' ? 'open' : 'closed'
            );
      ...
      serializer.addNodeFilter(accordionTag, (nodes) => {
        ...
        for (let i = 0; i < nodes.length; i++) {
          ...
          if (isAccordionDetailsNode(node)) {
            ...
            accordionNode.attr(
              'open',
              accordionNode.attr(accordionReadonlyCompensationAttribute) ===
                'open'
                ? 'open'
                : null
            );
