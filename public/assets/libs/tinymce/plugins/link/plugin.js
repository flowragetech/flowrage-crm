...
(function () {
  ...
  const hasFilesUploadHandler = (editor) =>
    isNonNullable(editor.options.get('files_upload_handler'));
  const hasDocumentsFileTypes = (editor) =>
    isNonNullable(editor.options.get('documents_file_types'));
  ...
  const blobToDataUri = (blob) =>
    new Promise((resolve, reject) => {
      ...
      reader.onerror = () => {
        reject(
          new Error(
            reader.error?.message ?? 'Failed to convert blob to a data url'
          )
        );
      };
  ...
  const collect = (editor, linkNode) =>
    LinkListOptions.getLinks(editor).then((links) => {
      const hasUploadPanel =
        hasFilesUploadHandler(editor) &&
        hasDocumentsFileTypes(editor) &&
        hasLinkUploadtab(editor);
  ...
  const dataUrlToBase64 = (dataUrl) =>
    Optional.from(dataUrl.split(',')[1]).getOr('');
  const changeFileInput = (helpers, api) => {
    ...
    head(data.fileinput).fold(
      ...
      (file) => {
        ...
        blobToDataUri(file)
          .then((dataUrl) => {
            const existingBlobInfo = helpers.getExistingBlobInfo(
              dataUrlToBase64(dataUrl),
              file.type
            );
            const blobInfo =
              existingBlobInfo && existingBlobInfo.filename() === file.name
                ? existingBlobInfo
                : helpers.createBlobCache(file, blobUri, dataUrl);
