...
export async function updateHomepage(values: any) {
  try {
    const existingHomepage = await withTimeout(
    ...
    revalidatePath('/dashboard/cms/homepage');
    return { success: true };
  } catch (error) {
    return { success: false, error: 'Failed to update homepage content' };
  }
}
